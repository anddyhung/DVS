...
import { JobInfo, JobDetail, JobInExpert, Milestone } from 'types/jobsinfo';
...
export const getHistoryJobs = () => async (dispatch: any) => {
  try {
    ...
    if (response.status === 200) {
      dispatch(setHistoryJobs(response.data));
    } else {
      dispatch(
        openSnackbar({
          open: true,
...
export const findJob = (keyword: string) => async (dispatch: any) => {
  try {
    const response = await axiosServices.post(`api/v1/job/`, { keyword });
    if (response.status === 200) {
      return response.data;
    } else {
      dispatch(
...
export const getExpertSearchedJobs = (keyword: string) => async (dispatch: any) => {
  try {
    const response = await axiosServices.post('/api/v1/job/expert/search', { keyword });
    if (response.status === 200) {
      dispatch(setSearchResult(response.data[0].activeJobs));
    } else {
      dispatch(
        openSnackbar({
          open: true,
...
interface JobState {
  ...
  expertSavedJobs: JobInfo[];
  expertSearchResults: JobInfo[];
  expertRecommendedJobs: JobInfo[];
...
export const jobSlice = createSlice({
  ...
  reducers: {
    ...
    setSavedJobs: (state, action: PayloadAction<JobInfo[] | any>) => {
      state.expertSavedJobs = action.payload;
    },
    setSearchResult: (state, action: PayloadAction<JobInfo[] | any>) => {
      state.expertSearchResults = action.payload;
    },
    setExpertRecommendedJobs: (state, action: PayloadAction<JobInfo[] | any>) => {

...
const AddEventFrom = (props: any) => {
  ...
  const EventSchema = Yup.object().shape({
    title: Yup.string().max(255).required('Project title is required'),
  ...
  const formik = useFormik({
    initialValues: getInitialValues(props.event!, props.range),
    ...
    onSubmit: (values, { setSubmitting }) => {
      try {
        ...
        if (props.event) {
          dispatch(updateEvent(props.event.id, newEvent));          
        }else{
    ...
    <FormikProvider value={formik}>
      <LocalizationProvider dateAdapter={AdapterDateFns}>
        <Form autoComplete="off" noValidate onSubmit={handleSubmit}>
          <DialogTitle>{props.event ? 'Edit Event' : 'Add Event'}</DialogTitle>
          <Divider />
          <DialogContent sx={{ p: 2.5 }}>
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <Stack spacing={1.25}>
                  <InputLabel htmlFor="cal-title">Project title</InputLabel>
                  <TextField
                    fullWidth
                    id="cal-title"
                    placeholder="Title"
              ...
              <Grid item xs={12}>
                <Stack spacing={1.25}>
                  <InputLabel htmlFor="cal-description">Meeting Agenda</InputLabel>
                  <TextField
                    fullWidth
                    id="cal-description"
                    multiline
              ...
              <Grid item xs={12}>
                <FormControlLabel control={<Switch checked={values.allDay} {...getFieldProps('allDay')} />} label="Video" />
              </Grid>

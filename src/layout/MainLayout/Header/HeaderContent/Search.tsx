...
const Search = () => {
  ...
  const handleKeyDown = () => {
    const searchResult = findJob(keyword);
    navigate('/client/search', { state: { searchResult: searchResult } });
  };
  const handleKeyWordChanged = (event: any) => {
  ...
  return (
    ...
    >
      <FormControl sx={{ width: { xs: '100%', md: '30rem' } }}>
        <InputBase
          size="small"
          style={{ paddingLeft: '10px' }}
          id="header-search"
          onChange={(event: any) => handleKeyWordChanged}
